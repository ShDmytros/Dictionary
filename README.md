...
з списку `dict`, у файл `dictionary.dict`. Для зберігання програма використовує 2 функції у класі `WorkingWithFiles`, 
...
`readData(map<string, string>& dict)` та під час кожної зміни списку `dict`, 
...
1. Якщо користувач підтвердив _(натиснув `1`)_, програма виводить:
   ...
   та програма видаляє всі значення в списку `data`.
...
- якщо був введений 1
- або в копії списку `dict`, закінчилися слова, тоді в цьому випадку, нам виводиться:
  >Alright, you guessed all words!

----------------

## Взагалому це весь основний функціонал програми, для неї ще будуть доповнення та оновлення, тому програма не буде стояти на одному місці і буде ще багато оновлень.
